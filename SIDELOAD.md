...
| **Continue Widget Extension** | **Shipped (Continue + Next Up)** | Embedded as `PlugIns/Silveran Reader Widgets.appex` with **four** kinds: light/dark × medium/large (`inkamp.continue.{light,dark}.{medium,large}.v1`). Remove any old Continue / Continue + Up next tile (`InkAmpContinueWidget`, `inkamp.continue.upnext.v1`, v3/v4), open ink+amp once, then add one of the four new tiles. App Group still resolves `ALTAppGroups` → `SILVERAN_WIDGET_APP_GROUP` → literal. No playback transport on the Home Screen tiles — Continue / Up Next / Browse Queue are deep links. Costs one extra App ID + one App Group. See `docs/CONTINUE_WIDGET.md`. |
