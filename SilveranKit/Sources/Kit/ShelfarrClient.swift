...
public struct ShelfarrClient: Sendable {
    ...
    private static func transportError(_ error: Error, original: URL?) -> ShelfarrError {
        ...
        let rawCode = urlCode?.rawValue ?? nsError.code
        ...
        if urlCode == .timedOut || rawCode == URLError.Code.timedOut.rawValue {
            return .transportError("Shelfarr timed out. Check the server address and try again.")
        }
    ...
    static func redactSecrets(_ message: String) -> String {
        var text = message
        if let expression = try? NSRegularExpression(pattern: #"shf_[A-Za-z0-9_-]+"#) {
        ...
        if let bearer = try? NSRegularExpression(pattern: #"(?i)Bearer\s+[A-Za-z0-9._=-]+"#) {
            let range = NSRange(text.startIndex..., in: text)
            text = bearer.stringByReplacingMatches(
                in: text,
                range: range,
