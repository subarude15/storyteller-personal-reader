...
struct BookFormatLinkTests {
    @Test func ranksPunctuationCaseAndSubtitleWithoutAutoLinking() {
        ...
        #expect(Set(ranked.map(\.book.uuid)) == ["audio-isbn", "audio-close", "audio-sub", "audio-edition"])
        #expect(ranked.allSatisfy { $0.book.id != currentWithISBN.id })
        let summaries = Dictionary(uniqueKeysWithValues: ranked.map { ($0.book.uuid, $0.editionSummary) })
    ...
    @Test func excludesIncompatibleAlreadyLinkedAndPodcasts() {
        let current = makeBook(uuid: "ebook", title: "Dune", authors: ["Frank Herbert"], ebook: true)
        let sameFormat = makeBook(uuid: "other-ebook", title: "Dune", authors: ["Frank Herbert"], ebook: true)
        ...
        let linkedElsewhere = makeBook(uuid: "taken", title: "Dune", authors: ["Frank Herbert"], audiobook: true)
        let partner = makeBook(uuid: "partner", title: "Other", audiobook: true)
        let good = makeBook(uuid: "good", title: "Dune", authors: ["Frank Herbert"], audiobook: true)
    ...
    @Test func searchStillFindsLowerRankedEditions() {
        ...
        let match = makeBook(uuid: "audio", title: "Children of Dune", authors: ["Frank Herbert"], audiobook: true)
    ...
    @Test func groupingHidesOnlyTheLinkedRecord() {
        ...
        let otherEdition = makeBook(uuid: "edition", title: "Dune", subtitle: "Graphic novel", ebook: true)
        ...
        let actions = BookFormatGrouping.playbackActions(
            for: BookFormatGrouping.members(of: ebook.id, in: [ebook, audio, otherEdition], links: [link])
        )
    ...
    @Test func readAndListenAppearsOnlyWhenStorytellerSaysAligned() {
        let queued = makeBook(uuid: "q", title: "Dune", ebook: true, audiobook: true, readaloud: "QUEUED")
        let failed = makeBook(uuid: "f", title: "Dune", ebook: true, audiobook: true, readaloud: "ERROR")
        let ready = makeBook(uuid: "r", title: "Dune", ebook: true, audiobook: true, readaloud: "ALIGNED")
        ...
        #expect(BookFormatGrouping.playbackActions(for: [ready]).map(\.kind) == [.read, .listen, .readAndListen])
    ...
    @Test func freshServerDocumentGroupsWithoutHidingAnotherEdition() async throws {
        ...
        let decoded = try BookFormatLinkMerge.decodeDescription(encoded)
        let cache = FormatLinkCacheDouble()
        let transport = FormatLinkTransportDouble()
        await transport.setFetch(.document(decoded))
        ...
        let visible = BookFormatGrouping.visibleBooks([ebook, audio, edition], links: stored.activeLinks)
        #expect(visible.map(\.uuid) == ["ebook", "edition"])
    }
    ...
    @Test func missingSourceIsRejectedBeforePush() async {
        ...
        let audio = makeBook(uuid: "audio", title: "Dune", audiobook: true)
        let transport = FormatLinkTransportDouble()
        let coordinator = BookFormatLinkCoordinator(cache: FormatLinkCacheDouble(), transport: transport)
        let outcome = await coordinator.link(
            sourceID: "server",
            primary: ebook,
            other: audio,
    ...
    @Test func unlinkKeepsBothBooks() async {
        let ebook = makeBook(uuid: "ebook", title: "Dune", ebook: true)
        let audio = makeBook(uuid: "audio", title: "Dune", audiobook: true)
        let link = BookFormatLink(
            id: BookFormatLink.identifier(for: [ebook.id, audio.id]),
        ...
        let cache = FormatLinkCacheDouble()
        await cache.save(sourceID: "server", document: BookFormatLinkDocument(updatedAt: link.updatedAt, links: [link]))
        let transport = FormatLinkTransportDouble()
        await transport.setFetch(.document(BookFormatLinkDocument(updatedAt: link.updatedAt, links: [link])))
        let coordinator = BookFormatLinkCoordinator(cache: cache, transport: transport)
        ...
        #expect(document.links.contains { $0.removed && $0.members.contains(ebook.id) && $0.members.contains(audio.id) })
        let library = [ebook, audio]
    ...
    @Test func alignmentStartDoesNotClaimReady() async {
        ...
        let reuse = BookFormatLinkCoordinator(cache: FormatLinkCacheDouble(), transport: reuseTransport)
        ...
        let failed = BookFormatLinkCoordinator(cache: FormatLinkCacheDouble(), transport: failedTransport)
        ...
        let retry = BookFormatLinkCoordinator(cache: FormatLinkCacheDouble(), transport: retryTransport)
        let retryOutcome = await retry.retryAlignment(members: [
            makeBook(uuid: "broken", title: "Dune", ebook: true, audiobook: true, readaloud: "ERROR")
        ])
    ...
    @Test func tombstoneFromServerSurvivesRefresh() async {
        let ebook = makeBook(uuid: "ebook", title: "Dune", ebook: true)
        let audio = makeBook(uuid: "audio", title: "Dune", audiobook: true)
        ...
        await transport.setFetch(
            .document(BookFormatLinkDocument(updatedAt: remote.updatedAt, links: [remote]))
        )
        let coordinator = BookFormatLinkCoordinator(cache: cache, transport: transport)
        let active = await coordinator.refresh(sourceID: "server")
...
private func makeBook(
    ...
    BookMetadata(
        ...
        authors: authors.map {
            BookCreator(uuid: nil, id: nil, name: $0, fileAs: nil, role: "aut", createdAt: nil, updatedAt: nil)
        },
        narrators: nil,
        creators: nil,
        series: series,
        ...
        collections: nil,
        ebook: ebook
            ? BookAsset(uuid: uuid, filepath: "\(uuid).epub", missing: 0, createdAt: nil, updatedAt: nil) : nil,
        audiobook: audiobook
            ? BookAsset(uuid: uuid, filepath: "\(uuid).m4b", missing: 0, createdAt: nil, updatedAt: nil) : nil,
